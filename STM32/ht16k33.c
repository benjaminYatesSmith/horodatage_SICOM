...
void afficher_secondes(uint8_t secondes) {
    uint8_t dizaine = (secondes >> 4) & 0x0F;  // Extraction des dizaines (4 bits de poids fort)
    uint8_t unite = secondes & 0x0F;           // Extraction des unités (4 bits de poids faible)
...
//fonction qui affiche les milli-secondes (argument de la fonction est int entre 0 et 999)
// sur l'afficheur 7 segments au position 5 & 6 & 7 et on affiche un point à l'emplacement 4.

void afficher_millisec(uint16_t millis) {
    uint8_t centaine = (millis * 0x147B) >> 17;  // Division exacte par 100
    uint8_t dizaine = ((millis - (centaine * 100)) * 0xCD) >> 11;  // Division exacte par 10
    uint8_t unite = millis - (centaine * 100) - (dizaine * 10);  // millis - (centaine * 100 + dizaine * 10)
